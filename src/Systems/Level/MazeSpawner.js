...
import { MAZE_CONFIG } from '../../Config/MazeConfig.js';
...
export class MazeSpawner {
    ...
    spawnMazeRowFromConfig(y, config, allowMoving, allowSpikes, rowIndex = null, pattern = null, tintColor = null, enemyBudget = null, coinBudget = null) {
        ...
        const MIN_GAP = MAZE_CONFIG.MIN_GAP;
        ...
        const gapMargin = Math.min(MAZE_CONFIG.GAP_MARGIN_MAX, gapWidth * MAZE_CONFIG.GAP_MARGIN_RATIO);
        ...
        if (rowIndex >= 0 && enemySpawnChance > 0 && mazeBudget.spawned < mazeBudget.target && scene.patrolEnemies.countActive() < maxEnemiesRow) {
            ...
            // Removed delayedCall for predictable/immediate spawning
            ...
            for (let i = 0; i < enemiesToSpawn; i++) {
                ...
                let safeMin = segment.min + MAZE_CONFIG.ENEMY_SPAWN_SAFE_PADDING;
                let safeMax = segment.max - MAZE_CONFIG.ENEMY_SPAWN_SAFE_PADDING;
