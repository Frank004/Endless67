...
import { MAZE_PATTERNS, MAZE_PATTERNS_EASY, MAZE_PATTERNS_MEDIUM, MAZE_PATTERNS_HARD, MAZE_ROW_HEIGHT } from '../data/MazePatterns.js';
...
export class Playground extends Game {
    ...
    create() {
        ...
        // --- REMOVE INITIAL PLATFORM FROM GAME.CREATE() ---
        // En devMode no queremos la plataforma inicial del juego normal
        if (this.startPlatform) {
            this.startPlatform.destroy();
            this.startPlatform = null;
        }

        // --- CLEAN UP SLOTS GENERATED BY GAME.CREATE() ---
        // El slotGenerator.init() se llama en Game.create(), limpiar los slots generados
        if (this.slotGenerator) {
            // Resetear el generador de slots
            this.slotGenerator.reset();
            // Deshabilitar la generación automática de slots
            this.slotGenerator.update = () => {
                // Do nothing - we want empty playground
            };
        }

        // --- CLEAN UP PLATFORMS GENERATED BY SLOT GENERATOR ---
        // Eliminar todas las plataformas generadas por el slot generator (excepto el piso permanente que crearemos)
        this.platforms.children.each((platform) => {
            if (platform && !platform.getData('isPermanentFloor')) {
                platform.destroy();
            }
        });
        ...
        this.categories = [
            ...
            },
            {
                icon: 'tornado', label: 'Mazes', items: [
                    ...
                    { icon: 'tornado', label: 'Maze 8 (Easy)', callback: (mode) => this.spawnSpecificMaze(7, mode), color: '#00ff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 9 (Easy)', callback: (mode) => this.spawnSpecificMaze(8, mode), color: '#00ff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 10 (Easy)', callback: (mode) => this.spawnSpecificMaze(9, mode), color: '#00ff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 11 (Easy)', callback: (mode) => this.spawnSpecificMaze(10, mode), color: '#00ff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 12 (Medium)', callback: (mode) => this.spawnSpecificMaze(11, mode), color: '#ffff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 13 (Hard)', callback: (mode) => this.spawnSpecificMaze(12, mode), color: '#ff0000', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 14 (Medium)', callback: (mode) => this.spawnSpecificMaze(13, mode), color: '#ffff00', type: 'dual' },
                    { icon: 'tornado', label: 'Maze 15 (Hard)', callback: (mode) => this.spawnSpecificMaze(14, mode), color: '#ff0000', type: 'dual' }
                ]
            },
            {
    ...
    spawnMaze(difficulty) {
        // Spawn maze a 160px arriba del jugador para que sea accesible
        const y = this.player.y - 160;
    ...
    spawnSpecificMaze(index, mode = 'clean') {
        // Spawn maze a 160px arriba del jugador para que sea accesible
        const slotYStart = this.player.y - 160;
        ...
        // Usar el SlotGenerator para generar el maze como un slot completo
        // Esto asegura que tenga la estructura correcta de slot con todas las características
        const config = SLOT_CONFIG.types.MAZE;
        const rowCount = pattern.length; // Usar el número de filas del patrón
        const rowHeight = config.rowHeight || MAZE_ROW_HEIGHT;
        const rowGap = config.rowGap || 100;
        const slotHeight = rowCount * rowHeight + (rowCount - 1) * rowGap;

        // Aplicar transformaciones según la configuración del slot
        const tWeights = config.transformWeights || { none: 1 };
        const r = Math.random();
        const mirrorX = r < (tWeights.mirrorX || 0);
        const mirrorY = !mirrorX && r < ((tWeights.mirrorX || 0) + (tWeights.mirrorY || 0));
        ...
        // Presupuesto de enemigos y coins (solo si mode es 'prepopulate')
        const enemyBudget = mode === 'prepopulate' ? {
            target: Phaser.Math.Between(1, 2),
            spawned: 0
        } : { target: 0, spawned: 0 };

        const coinBudget = mode === 'prepopulate' ? {
            bonus: 2,
            used: 0
        } : { bonus: 0, used: 0 };

        // Spawn maze rows como un slot completo
        for (let row = 0; row < rowCount; row++) {
            const rowY = slotYStart - (row * (rowHeight + rowGap));
            let rowConfig = rowsToUse[row % rowsToUse.length];
            
            // Aplicar mirrorX si corresponde
            if (mirrorX) {
                rowConfig = { ...rowConfig };
            ...
            // Spawn con configuración según el modo
            const allowSpikes = mode === 'prepopulate';
            this.levelManager.spawnMazeRowFromConfig(
                rowY,
                rowConfig,
                false, // allowMoving
                allowSpikes, // allowSpikes / enemies
                row,
                rowsToUse,
                null, // tintColor
                enemyBudget,
